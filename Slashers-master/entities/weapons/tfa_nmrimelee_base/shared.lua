...
function SWEP:PrimarySlash()
	...
	if self.meleedmgcvar then
		pain = pain * self.meleedmgcvar:GetFloat()
	end

	local owner = self.Owner
	owner.tmpdmgtype = self.Primary.Blunt and DMG_CLUB or DMG_SLASH

	owner:LagCompensation(true)
	local tr = util.TraceHull({
		start = owner:GetShootPos(),
		endpos = owner:GetShootPos() + owner:EyeAngles():Forward() * self.Primary.Reach,
		filter = owner,
	...
	if tr.Hit then
		if not tr.HitSky then
			self:HitSound(tr.MatType, false, tr.Entity:IsNPC() or tr.Entity:IsPlayer())
		end
		if SERVER and IsValid(tr.Entity) then
			...
			dmg:SetDamageForce(owner:GetAimVector() * (pain * 0.25))
			dmg:SetDamage(pain)
			dmg:SetDamageType(owner.tmpdmgtype or DMG_SLASH)
			tr.Entity:TakeDamageInfo(dmg)
			self:ApplyForce(tr.Entity, pain, tr.HitPos, tr.PhysicsBone)
		end
	else
...
function SWEP:MotorSlash()
	...
	if game.GetTimeScale()>0.99 then
		...
		if slashtrace.Hit then
			...
			if SERVER and IsValid(slashtrace.Entity) then
				local dmg = DamageInfo()
...
function SWEP:SecondarySlash()
	...
	local owner = self.Owner
	owner.tmpdmgtype = self.Secondary.Blunt and DMG_CLUB or DMG_SLASH

	owner:LagCompensation(true)
	local tr = util.TraceHull({
		start = owner:GetShootPos(),
		endpos = owner:GetShootPos() + owner:EyeAngles():Forward() * self.Secondary.Reach,
		filter = owner,
		mins = Vector(-20, -20, -15),
		maxs = Vector(20, 20, 15)
	})
	...
	if tr.Hit then
		if not tr.HitSky then
			self:HitSound(tr.MatType, true, tr.Entity:IsNPC() or tr.Entity:IsPlayer())
		end
		if SERVER and IsValid(tr.Entity) then
			local dmg = DamageInfo()
			dmg:SetAttacker(owner)
