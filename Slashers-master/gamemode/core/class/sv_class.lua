...
function playermeta:SetupKiller()
	...
	self:AllowFlashlight(false)
	self.HasChosenCharacter = false
	self.HasChosenWeapon    = false
	...
	-- ATMOSPHERIC: hard-freeze the killer until they complete both setup stages
	self:Freeze(true)

	-- NOTE: Model, speed, and weapons are NOT applied here.
	-- They are set in sls_killer_selectchar (model/speed/hands) and
	-- sls_killer_selectweapon (weapon, then unfreeze).

	-- Stage 1: open character selection menu
